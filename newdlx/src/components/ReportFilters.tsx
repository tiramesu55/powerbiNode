...
import { FilterBuilder, IFilterConfiguration, IFilterValues } from '../PowerBi/FilterBuilder';
...
export default function ReportFilters(props: IFilterProps) {
    ...
    const handleApplyFilter = () => {
        const powerBiFilterArray = filterBuilder.buildReportFilters(filterValues, props.filterConfiguration);
    ...
    const handleFilterValueChange = (name: string) => (event: { target: { value: any } }) => {
        setFilterValues({ ...filterValues, [name]: event.target.value });
    };

    // const handleFilterDateValueChange = (name: string) => (value: { $d: Date }) => {
    //     if (value != null) {
                    ...
                    >
                        {locationOptions.map((loc) => (
                            <MenuItem key={loc} value={loc}>
                                <Checkbox color="primary" checked={filterValues.locations.indexOf(loc) > -1} />
