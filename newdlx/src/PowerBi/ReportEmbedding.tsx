...
export class ReportEmbedding {
    ...
    private getReportEmbedModelFromResponse(response: Response): Promise<IReportEmbedModel> {
        if (response.status === 200) {
            return response.json();
        } else throw 'Error fetching report embed model';
    }

    private buildReportEmbedConfiguration(embedModel: IReportEmbedModel): IEmbedConfiguration {
        return {
            id: embedModel.id,
